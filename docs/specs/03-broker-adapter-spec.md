...
- OpenD 是否运行在同一台服务器。
- OpenD 端口、解锁方式、重连策略。
- OpenD 日志和 BrokerGate 审计日志的关联方式。
...
- SDK adapter：BrokerGate 主路径，用于账户快照、持仓、风控和确认后下单。
- Hosted MCP：长桥官方 AI 入口，只作为参考或用户单独使用，不作为 BrokerGate 的交易执行后端。
...
长桥有 `estimate_max_purchase_quantity()`，可以作为券商特有的额外 precheck，但不能替代 BrokerGate 的统一购买力规则。详细方案见 `docs/guides/longbridge-openapi-integration.md`。
